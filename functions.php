...
function startup_setup() {
	/*
		...
		* to change 'startup' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'startup' ),
			'menu-footer' => esc_html__( 'Footer', 'startup' ),
		)
	);
 ...
 */
function startup_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'startup_content_width', 640 );
}
add_action( 'after_setup_theme', 'startup_content_width', 0 );
...
function startup_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'startup' ),
...
function startup_scripts() {
	...
	wp_style_add_data( 'startup-style', 'rtl', 'replace' );

	wp_enqueue_style( 'preconnect', '//fonts.googleapis.com', array(), _S_VERSION, true );
	wp_enqueue_style( 'preconnecte', '//fonts.gstatic.com', array(), _S_VERSION, true );


	wp_enqueue_style( 'preconnect','//fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800&family=Rubik:wght@400;500;600;700&display=swap', array(), _S_VERSION, true );

	wp_enqueue_style( 'preconnect', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, 'all'  );


	wp_enqueue_style( 'awesome', get_template_directory_uri() . '/assets/css/font-awesome.min.css', array(), _S_VERSION, 'all'  );
	wp_enqueue_style( 'carousel', get_template_directory_uri() . '/assets/css/owl.carousel.min.css', array(), _S_VERSION, 'all'  );
	...
	wp_enqueue_script( 'carousel', get_template_directory_uri() . '/assets/js/owl.carousel.min.js', array('jquery'), _S_VERSION, true );
...
// resigtation widget development 
require get_template_directory() . '/inc/sidebar-resigtation.php';
